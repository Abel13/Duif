...
sete dias, prepara novas ofertas idempotentemente, e gera no máximo 20 quests por execução.
...
contagens e códigos de falha. A IA recebe o contexto ficcional aprovado do template — solicitante,
posto, carga, problema e objetivo — e publica uma quest estruturada com briefing, objetivo de ida
e registro de retorno; a cidade é apenas o local da entrega, nunca uma fonte de alegações reais.
Se a OpenAI estiver indisponível, recusar a solicitação ou devolver um schema inválido, uma quest
localizada determinística do mesmo template é publicada para o primeiro destino validado. O banco
sempre escolhe as cidades GeoNames candidatas e revalida alcance, capacidade, propriedade e
expiração no despacho.
