...
`weather-travel-resolver` is invoked every três horas. Configure os segredos Vault `duif_project_url` e `duif_service_role_key` para o agendador, além de `OPEN_METEO_BASE_URL` e, em produção, um `OPEN_METEO_API_KEY` aprovado para a função. Um endpoint público sem chave só é aceito quando a URL Supabase é local. Se configuração, timeout, validação ou provedor falharem, os snapshots virtuais existentes continuam autoritativos e a viagem segue normalmente.
