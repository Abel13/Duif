...
begin
  ...
  if asset_count<>54 then raise exception 'All 54 memorable places must be active, found %',asset_count; end if;
  if exists(select 1 from public.world_landmark_catalog where active and (rules_version<>1 or eligibility_radius_km<>25 or minimum_zoom<>8 or icon_size_px<>56 or sort_order not between 0 and 54)) then
    raise exception 'Memorable-place catalog contract is invalid';
  end if;
  if (select count(distinct sort_order) from public.world_landmark_catalog where active)<>54 then raise exception 'Memorable-place sort orders are incomplete'; end if;
  ...
  if postcard_count<>54 then raise exception 'Memorable-place postcard pair is incomplete, found %',postcard_count; end if;
