...
cross join (values
  ...
  ('00000000-0000-4000-8000-000000001006'::uuid,'Tutorial visitor')
) fixture(id,name)
...
select gen_random_uuid(),delivery.id,case when delivery.reward_seed='active-visitor-5' then 'postcard'::public.correspondence_type else 'letter'::public.correspondence_type end,case when delivery.reward_seed<>'active-visitor-5' then 'Private letter' end,case when delivery.reward_seed='active-visitor-5' then 'Postcard' end
...
values('00000000-0000-4000-8000-000000002001','00000000-0000-4000-8000-000000000101','00000000-0000-4000-8000-000000000001','Reply',now()+interval '10 minutes');

set local role authenticated;
select set_config('request.jwt.claim.sub','10000000-0000-4000-8000-000000000101',true);
do $$ begin if exists(select 1 from public.list_active_postal_visitors() where delivery_id='00000000-0000-4000-8000-000000002001') then raise exception 'Answered visitor remained active'; end if; end $$;
