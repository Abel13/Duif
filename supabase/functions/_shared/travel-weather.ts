export type NormalizedWeather = { weatherCode: number; isDay: boolean; windSpeedKmh: number; windGustKmh: number; temperatureC: number };
...
export function normalizeOpenMeteo(payload: unknown, blockStart: string): NormalizedWeather | null {
  ...
  const weatherCode=read("weather_code"), isDay=read("is_day"), windSpeedKmh=read("wind_speed_10m"), windGustKmh=read("wind_gusts_10m"), temperatureC=read("temperature_2m");
  if (index<0 || !Number.isInteger(weatherCode) || weatherCode<0 || weatherCode>99 || ![0,1].includes(isDay) || !Number.isFinite(windSpeedKmh) || windSpeedKmh<0 || !Number.isFinite(windGustKmh) || windGustKmh<0 || !Number.isFinite(temperatureC) || temperatureC < -90 || temperatureC > 70) return null;
  return { weatherCode, isDay:isDay===1,windSpeedKmh,windGustKmh,temperatureC };
}
...
export function openMeteoUrl(baseUrl: string, latitude: number, longitude: number, apiKey?: string) {
  ...
  url.searchParams.set("hourly","weather_code,is_day,wind_speed_10m,wind_gusts_10m,temperature_2m"); url.searchParams.set("forecast_days","4"); url.searchParams.set("timezone","UTC"); url.searchParams.set("wind_speed_unit","kmh"); url.searchParams.set("temperature_unit","celsius");
