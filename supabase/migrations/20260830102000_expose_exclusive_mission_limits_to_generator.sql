-- The generator receives only the already-authoritative cargo limit, never a
-- mutable reward or private coordinate. This is a follow-up because the base
...
returns table(mission_id uuid, lease_token uuid, mascot_name text, origin_hint text, cargo_slots integer, candidates jsonb)
...
begin
  ...
  select mission.id, mission.generation_lease_token, mascot.name,
    concat_ws(' · ', nullif(profile.postal_base_city,''), nullif(profile.postal_base_country,'')), mission.cargo_slots, mission.candidate_destinations
