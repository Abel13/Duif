-- Reveal the courier only after the recipient opens the correspondence.
...
begin
  ...
  return query
  select d.id,'outbound',d.outbound_arrival_at,c.correspondence_type::text,(o.delivery_id is not null),case when o.delivery_id is not null then m.name end,case when o.delivery_id is not null then m.appearance->>'portraitAssetKey' end,case when o.delivery_id is not null then p.display_name end,case when o.delivery_id is not null then p.id end,case when o.delivery_id is not null then coalesce(d.origin_place_label,d.origin_label_key) end,case when o.delivery_id is not null then c.letter_text end,case when o.delivery_id is not null then c.postcard_message end,c.postcard_catalog_key,c.metadata->'postcard'->>'nameKey',c.metadata->'postcard'->>'artworkAssetKey',c.sticker_ids,coalesce((select array_agg(s.artwork_asset_key order by x.ordinality) from unnest(c.sticker_ids) with ordinality x(catalog_key,ordinality) join public.official_stickers s on s.catalog_key=x.catalog_key),array[]::text[]),d.outbound_arrival_at+interval '60 minutes',exists(select 1 from public.delivery_return_replies r where r.delivery_id=d.id),c.metadata#>>'{postalFinishing,stamp,assetKey}',coalesce(c.metadata#>>'{postalFinishing,postmark,key}','postalMark.postalCancel'),coalesce(c.metadata#>>'{postalFinishing,postmark,model}','classic'),coalesce(c.metadata#>>'{postalFinishing,postmark,color}','brown'),c.metadata#>>'{postalFinishing,postmark,city}',c.metadata#>>'{postalFinishing,postmark,country}',nullif(c.metadata#>>'{postalFinishing,postmark,date}','')::date
  from public.deliveries d join public.delivery_correspondence_contents c on c.delivery_id=d.id join public.profiles p on p.id=d.sender_profile_id join public.player_mascots m on m.id=d.mascot_id left join public.delivery_mailbox_opens o on o.delivery_id=d.id and o.profile_id=me and o.direction='outbound'
  where d.receiver_profile_id=me and not d.is_tutorial and d.outbound_arrival_at<=now()
  union all
  select d.id,'return',d.return_arrival_at,'letter',(o.delivery_id is not null),case when o.delivery_id is not null then m.name end,case when o.delivery_id is not null then m.appearance->>'portraitAssetKey' end,case when o.delivery_id is not null then p.display_name end,case when o.delivery_id is not null then p.id end,case when o.delivery_id is not null then coalesce(d.destination_place_label,d.destination_label_key) end,case when o.delivery_id is not null then r.letter_text end,null,null,null,null,array[]::text[],array[]::text[],null,true,r.metadata#>>'{postalFinishing,stamp,assetKey}',coalesce(r.metadata#>>'{postalFinishing,postmark,key}','postalMark.postalCancel'),coalesce(r.metadata#>>'{postalFinishing,postmark,model}','classic'),coalesce(r.metadata#>>'{postalFinishing,postmark,color}','brown'),r.metadata#>>'{postalFinishing,postmark,city}',r.metadata#>>'{postalFinishing,postmark,country}',nullif(r.metadata#>>'{postalFinishing,postmark,date}','')::date
  from public.deliveries d join public.delivery_return_replies r on r.delivery_id=d.id join public.profiles p on p.id=r.sender_profile_id join public.player_mascots m on m.id=d.mascot_id left join public.delivery_mailbox_opens o on o.delivery_id=d.id and o.profile_id=me and o.direction='return'
...
revoke all on function public.list_received_correspondence(),public.open_received_correspondence(uuid,text) from public,anon;
