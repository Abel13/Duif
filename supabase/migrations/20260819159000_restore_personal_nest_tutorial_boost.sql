-- The personal-nest route replacement retained an older tutorial timeline. Restore the approved
-- first-journey timings without changing the dynamic origin/destination calculation.
do $$
...
begin
  ...
  if definition not like '%30 seconds%' or definition not like '%2 minutes 30 seconds%'
    or definition not like '%3 minutes%' or definition not like '%5 minutes%' or definition not like '%tutorialBoost%' then
    raise exception 'Could not restore the personal-nest tutorial boost';
  end if;
  execute definition;
end $$;
