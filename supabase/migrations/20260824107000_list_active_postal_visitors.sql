create or replace function public.list_active_postal_visitors()
returns table(
  delivery_id uuid,
  mascot_id uuid,
  mascot_name text,
...
as $$
  select d.id,m.id,m.name,m.appearance->>'portraitAssetKey',d.return_start_at
  ...
  where receiver.auth_user_id=auth.uid()
    ...
    and not exists(select 1 from public.delivery_return_replies reply where reply.delivery_id=d.id)
