...
container_id=""; container_cities_file=""; container_import_sql=""
cleanup() {
  if [[ -n "$container_id" ]]; then
    docker exec "$container_id" rm -f "$container_cities_file" "$container_import_sql" >/dev/null 2>&1 || true
  fi
  rm -rf "$working_dir"
}
trap cleanup exit
archive="$working_dir/cities15000.zip"
...
sed "s|__CITIES_FILE__|$cities_file|g" supabase/admin/import_geonames_cities.sql > "$host_import_sql"
...
sed "s|__CITIES_FILE__|$container_cities_file|g" supabase/admin/import_geonames_cities.sql > "$host_import_sql"
