...
export function MailboxPage() {
  ...
  return <PageShell hasBottomNav>
    ...
    {selected?.correspondenceType === "postcard" && selected.postcardAssetKey ? <PostcardDialog backLabel={t("tutorial.postcard.back")} closeLabel={t("mailbox.closeLetter")} content={{ title: correspondenceTitle(selected, t), frontAssetKey: selected.postcardAssetKey, frontAlt: correspondenceTitle(selected, t), message: correspondencePreview(selected, t), senderName: selected.senderName ?? "", deliveredBy: `${t("mailbox.from")} ${selected.senderName ?? ""}`, originTitle: t("mailbox.from"), originLabel: selected.originLabel ?? t("common.unavailable"), destinationTitle: t("mailbox.open"), destinationLabel: formatDate(selected.arrivedAt) }} flipLabel={t("tutorial.postcard.flip")} frontLabel={t("tutorial.postcard.front")} onClose={() => setSelectedKey(undefined)} open /> : null}
