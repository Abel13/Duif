...
import { AssetImage, LetterDialog, SketchPanel, StampButton } from "../../components/ui";
...
export function MailboxPage() {
  ...
  return <PageShell hasBottomNav>
    ...
    {selected && selected.correspondenceType !== "letter" ? <dialog className={styles.contentDialog} open><button className={styles.dialogClose} type="button" onClick={() => setSelectedKey(undefined)}>{t("mailbox.closeLetter")}</button><article className={styles.contentPaper}>{selected.correspondenceType === "postcard" && selected.postcardAssetKey ? <AssetImage alt={selected.postcardNameKey ? t(selected.postcardNameKey) : t("correspondence.postcard.name")} assetKey={selected.postcardAssetKey} className={styles.postcardArt}><i /></AssetImage> : null}<h2>{correspondenceTitle(selected, t)}</h2><p>{correspondencePreview(selected, t)}</p><footer>{selected.senderName}</footer>{canReply ? <ReplyComposer text={replyText} disabled={isReplying} onChange={setReplyText} onSend={() => void sendReply()} /> : null}</article></dialog> : null}
    <AppBottomNav />
  </PageShell>;
}
