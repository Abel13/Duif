...
export function FriendsPage() {
  ...
  return <PageShell hasBottomNav>
    <div className={styles.shell}>
      ...
      {hasFriends ? <section className={styles.grid} aria-label={t("friends.title")}>{connections.accepted.map((friend) => <article className={styles.friendCard} key={friend.id}><ItemCard label={formatPostalLocationLabel(friend)} title={friend.displayName} /><FriendshipProgress exchangeCount={friend.exchangeCount} variant="compact" /><dl className={styles.stats}><div><dt>{t("friends.exchangeCount")}</dt><dd>{friend.exchangeCount}</dd></div></dl><div className={styles.actions}><Link className={styles.secondaryLink} to={`/friends/${friend.profileId}`}>{t("friends.viewProfile")}</Link><Link className={styles.primaryLink} to={`/send?friendId=${friend.profileId}`}>{t("friends.quickSend")}</Link></div></article>)}</section> : null}
    </div><AppBottomNav />
  </PageShell>;
}
