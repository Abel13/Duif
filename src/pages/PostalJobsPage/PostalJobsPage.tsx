...
export function PostalJobsPage() {
  ...
  return <PageShell><main className={styles.page}><header><span>{t("postalJobs.eyebrow")}</span><h1>{t("postalJobs.title")}</h1><p>{t("postalJobs.description")}</p></header>{isLoading ? <p>{t("common.loading")}</p> : <section className={styles.list}>{mascots.map((mascot) => { const job = offers[mascot.id]; return <article className={styles.job} key={mascot.id}><p>{mascot.name}</p>{job ? <><h2>{t(job.template.title_key as TranslationKey)}</h2><p>{t(job.template.description_key as TranslationKey)}</p><dl><div><dt>{t("postalJobs.distance")}</dt><dd>{job.template.min_distance_km}–{job.template.max_distance_km} km</dd></div><div><dt>{t("postalJobs.cargo")}</dt><dd>{job.template.cargo_slots}</dd></div><div><dt>{t("postalJobs.seeds")}</dt><dd>{job.template.seed_reward}</dd></div></dl>{job.offer.status === "accepted" ? <StampButton disabled={busy === mascot.id || Boolean(mascot.currentDelivery)} onClick={() => void dispatch(mascot.id, job.offer.id)}>{t("postalJobs.depart")}</StampButton> : <div className={styles.actions}><StampButton disabled={busy === mascot.id || Boolean(mascot.currentDelivery)} onClick={() => void accept(mascot.id, job.offer.id)}>{t("postalJobs.accept")}</StampButton><StampButton disabled={busy === mascot.id || job.replacementsRemaining === 0} onClick={() => void replace(mascot.id)} variant="secondary">{t("postalJobs.replace")} ({job.replacementsRemaining})</StampButton></div>}</> : <p>{t("common.loading")}</p>}</article>; })}</section>}{error ? <p className={styles.error}>{t("postalJobs.error")}</p> : null}</main></PageShell>;
}
