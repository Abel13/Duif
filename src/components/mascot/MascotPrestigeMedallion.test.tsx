...
describe("MascotPrestigeMedallion",()=>{
 it("uses a circular framed portrait with accessible text",()=>{const html=renderToStaticMarkup(<MascotPrestigeMedallion alt="Nuvem — Primeiro Horizonte" borderAssetKey="prestige.border.firstHorizon" portraitAssetKey="mascot.portrait.nuvem"/>);expect(html).toContain('role="img"');expect(html).toContain('aria-label="Nuvem — Primeiro Horizonte"');expect(html).toContain("portrait");expect(html).toContain("border");expect(html).toContain("sizer")});
