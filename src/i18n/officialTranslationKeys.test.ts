...
import inauguralPostcardMigration from "../../supabase/migrations/20260719207100_register_inaugural_postcard.sql?raw";
import tutorialMigration from "../../supabase/migrations/20260719202000_mandatory_tutorial_delivery.sql?raw";
...
describe("official translation manifest", () => {
  ...
  it("matches the translation-key registry installed by the database migration", () => {
    const registryStatements = [catalogMigration, assetMigration, tutorialMigration, inauguralPostcardMigration].flatMap((migration) =>
      [...migration.matchAll(/insert into public\.official_translation_keys[\s\S]*?on conflict \(translation_key\) do nothing;/g)]
        .map((match) => match[0]),
    ).join("\n");
    const databaseKeys = [...new Set([...registryStatements.matchAll(/\('([^']+)'\)/g)]
